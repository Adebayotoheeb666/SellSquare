...
import { addSales, getBusiness, updateSales } from "../../../../services/authService";
...
export default function EditSales({ handleCancel, sale }) {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const business = useSelector((state) => state.auth.business);
  const [showPassword, setShowPassword] = useState(false);
  const [formData, setFormData] = useState(sale);
  const [assignToDefaultStore, setAssignToDefaultStore] = useState(
    (sale?.branchAssignments || []).some((a) => a.storeId === business?._id)
  );
  ...
  const editSalesInfo = async (e) => {
    ...
    try {
      ...
      await executeWithToast(
        (async () => {
          const data = await updateSales(userData);
        ...
        {
          loading: "Updating sales rep...",
          success: "Sales rep updated successfully!",
          error: (err) => err.message || "Failed to update sales rep.",
        }
      );
    } catch (error) {
  ...
  return (
    <div className="add-new-sales" onClick={handleCancel}>
      ...
      <div className="new-sales-data" onClick={(e) => e.stopPropagation()}>
        <form onSubmit={(e) => editSalesInfo(e)}>
